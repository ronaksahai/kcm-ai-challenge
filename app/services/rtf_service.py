...
def _strip_html_to_text(text: str) -> str:
    ...
    tr_blocks = re.split(r'(?i)<tr[^>]*>', text)
    new_text_parts = [tr_blocks[0]]
    in_table = False
    
    for block in tr_blocks[1:]:
        match = re.search(r'(?i)</tr>|</?(?:table|thead|tbody|tfoot)[^>]*>', block)
        if match:
        ...
        cell_blocks = re.split(r'(?i)<t[dh][^>]*>', row_content)
        clean_cells = []
        
        pre_cell_text = re.sub(r'<[^>]+>', ' ', cell_blocks[0]).strip()
        
        for idx, cell_html in enumerate(cell_blocks[1:]):
            cell_text = re.split(r'(?i)</t[dh]>', cell_html)[0]
            clean_str = re.sub(r'<[^>]+>', ' ', cell_text).strip()
        ...
        after_clean = re.sub(r'(?i)</?(?:table|thead|tbody|tfoot)[^>]*>', '', after_row)
        after_clean = re.sub(r'<!--.*?-->', '', after_clean, flags=re.DOTALL)
